...
class Trainer(BaseTrainer):
    ...
    def train(self, n_epochs=350,
        ...
        for epoch in range(n_epochs):
            ...
            backup = self._decide_checkpoint_backup(
                rcd['train_acc'], rcd['AMI'], n_pass, eps=eps, backup_stride=backup_stride)
            ...
            if rcd['AMI'] > max_ami:
                ...
                cur_stable_epoch += 1
            if cur_stable_epoch > epoch_limit:
                print("The AMI didn't increase for the last 100 epohs, early stopping!")
                break
    ...
    def train_minibatch(self, n_epochs=100,
        ...
        optimizer = torch.optim.Adam([
            {'params': model.parameters(), 'lr':3e-4, 'weight_decay':1e-2}])
        print(f" start training (device='{device}') ".center(60, '='))
        print('It will take minutes from epoch 0 to 100, please wait!')
        for epoch in range(n_epochs):
            model.train()
            self._cur_epoch += 1
            ...
            backup = self._decide_checkpoint_backup(rcd['train_acc'], rcd['AMI'], n_pass, eps=eps)
            ...
            if rcd['AMI'] > max_ami:
                ...
                cur_stable_epoch += 1
            if (cur_stable_epoch>epoch_limit) and (self._cur_epoch>500):
                print("The AMI didn't increase for the last 100 epohs, early stopping!")
                break
    ...
    def _decide_checkpoint_backup(
            self, train_acc, ami, n_pass, eps=1e-4, backup_stride=1000):

        backup = False
        if (self._cur_epoch >= n_pass - 1) and (train_acc > 0.9):
