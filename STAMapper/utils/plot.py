...
def plot_contingency_mat(
    ...
    ax = heatmap(contmat, figsize=figsize, ax=ax, fp=fp, cmap=cmap, **kwds)
...
def heatmap_probas(
    ...
    ax2.pcolor(pix_lbs, cmap=custom_cmap)
    for _ax in [ax1, ax2, ]:
        _ax.grid(False)
...
def _prepare_for_nxplot(
        ...
        colors=None):
    """
    g: a multipartite graph with 3 layers
    """
    if colors is None:
        colors = ('pink', 'lightblue', 'pink')
