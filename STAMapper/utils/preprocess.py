...
def quick_preprocess(
    ...
    if hvgs is None:
        ...
        _adata = _adata.copy()  # detach form view-data
    # 3: z-score 
...
def find_cells_with_no_edges(filtered_scnet):
    """
    Find cells with no KNN edges in sc data.
    Parameters
    ----------
    filtered_scnet : csr_matrix

    Returns
    -------
    cells_with_no_edges : idx of cells with no KNN edges in sc data.

    """
    cells_with_no_edges = []
    for idx in range(filtered_scnet.shape[0]):
        _, nonzero_cols, _ = find(filtered_scnet[idx])
        if len(nonzero_cols) == 0:
            cells_with_no_edges.append(idx)
...
def filter_celltype_KNN(adata_sc, came_inputs, adata1, key_class1='celltype'):
    ...
    row, col = scnet1.nonzero()
    mask = ct_array[row] == ct_array[col]
    filtered_data = scnet1.data[mask]
    filtered_row = row[mask]
    filtered_col = col[mask]
    result_array = sparse.csr_matrix((filtered_data, (filtered_row, filtered_col)), shape=scnet1.shape)
    filtered_scnet = result_array
    
    came_inputs['scnets'][0] = filtered_scnet
    adata1.obsp['connectivities'] = filtered_scnet
    
    return adata1, came_inputs, filtered_scnet

def calc_scores(folder_path, key_class2=None):
    max_ami_list = []
    test_acc_list = []
    macrof1_list = []
    weightf1_list = []
    ...
    for folder in folds:
        file_path = os.path.join(folder_path, folder, 'train_logs.csv')
        temp_df = pd.read_csv(file_path)
        filtered_df = temp_df.iloc[100:]
        max_ami_index = filtered_df['AMI'].idxmax()
        max_ami = filtered_df.loc[max_ami_index, 'AMI']
        test_acc = filtered_df.loc[max_ami_index, 'test_acc']
        macrof1 = filtered_df.loc[max_ami_index, 'macroF1']
        weightf1 = filtered_df.loc[max_ami_index, 'weightedF1']
        max_ami_list.append(max_ami)
        test_acc_list.append(test_acc)
        macrof1_list.append(macrof1)
        weightf1_list.append(weightf1)
        
    max_ami_max_index = max_ami_list.index(max(max_ami_list))
    max_test_acc = test_acc_list[max_ami_max_index]
    max_macrof1 = macrof1_list[max_ami_max_index]
    max_weightf1 = weightf1_list[max_ami_max_index]

    return max_ami_max_index, max_test_acc, max_macrof1, max_weightf1
