...
class AlignedDataPair(object):
    ...
    **kwds:
    ...
    def __init__(
            ...
            oo_spa_adj: Optional[sparse.spmatrix] = None,
        ...
        self.set_oo_spa_adj(oo_spa_adj)
    ...
    def get_obs_anno(self,
        ...
        if concat:
            if type(anno2)==pd.core.frame.DataFrame:
                return np.array(anno1.tolist() + anno2.tolist())
            else:
                return np.array(anno1.tolist() + list(anno2))
        else:
            return anno1, anno2
    ...
    def make_whole_net(self, selfloop_o=True, selfloop_v=True, ):
        ...
        if self._oo_spa_adj is not None:
            edge_dict[(ntypes['o'], etypes['oo_spa'], ntypes['o'])] = self._oo_spa_adj
        if selfloop_o:
            edge_dict[
    ...
    def set_oo_spa_adj(self, oo_spa_adj=None):
        if oo_spa_adj is None:
            self._oo_spa_adj = None
        elif sparse.isspmatrix(oo_spa_adj):
            #print('make spanet')
            mat_list = [sparse.csr_matrix((self.n_obs1,self.n_obs1)), oo_spa_adj]
            self._oo_spa_adj = sparse.block_diag(mat_list)
        else:
            raise ValueError(
                'if provided, `oo_spa_adj` should be either a scipy.spmatrix ')
