...
class HiddenRRGCN(nn.Module):
    ...
    def __init__(self,
        ...
        kwdicts = [
            (('gene', 'expressed_by', 'cell'), 'gcn', mod_params['gcn']),
            (('cell', 'self_loop_cell', 'cell'), 'gcn', mod_params['gcn']),
            (('cell', 'similar_to', 'cell'), 'gcn', mod_params['gcn']),
            (('cell', 'express', 'gene'), 'gcn', mod_params['gcn']),
            (('gene', 'self_loop_gene', 'gene'), 'gcn', mod_params['gcn']),
        ]
        self.layers = nn.ModuleList()
        # h2h
        '''
        ...
        '''
        self.shared_layer = BaseMixConvLayer(
    ...
    def forward(
        ...
        h_dict = self.layers[-1](graphs[-1], h_dict,  **kwds)
        self.hidden_states.append(h_dict)
